...
def resolve_conv_adain_fused_model_path(model_path: Path, settings: Settings) -> Path:
    if not settings.onnx_conv_adain_fusion:
        return model_path

    if settings.onnx_conv_adain_custom_op_library is None:
        raise ValueError(
            "FASTKOKORO_ONNX_CONV_ADAIN_CUSTOM_OP_LIBRARY is required when "
            "FASTKOKORO_ONNX_CONV_ADAIN_FUSION is enabled"
        )
    if not settings.onnx_conv_adain_custom_op_library.exists():
        raise FileNotFoundError(settings.onnx_conv_adain_custom_op_library)

    if settings.onnx_conv_adain_model_path is not None:
        if not settings.onnx_conv_adain_model_path.exists():
            raise FileNotFoundError(settings.onnx_conv_adain_model_path)
        return settings.onnx_conv_adain_model_path

    cache_path = _default_conv_adain_cache_path(model_path, settings)
    if cache_path.exists():
        return cache_path

    cache_path.parent.mkdir(parents=True, exist_ok=True)
    fused = fuse_generator_conv_adain(model_path, cache_path)
    logger.info(
        "Generated ConvAdaIN-fused ONNX model: path=%s fused=%s",
        cache_path,
        fused,
    )
    return cache_path
...
def _default_conv_adain_cache_path(model_path: Path, settings: Settings) -> Path:
    digest = hashlib.sha256(str(model_path.resolve()).encode("utf-8")).hexdigest()[:12]
    stem = model_path.name.removesuffix(".onnx")
    return settings.cache_dir / "onnx" / f"{stem}.conv-adain.{digest}.onnx"
...
def fuse_generator_conv_adain(input_path: Path, output_path: Path) -> int:
    model = onnx.load(input_path)
    consumers: dict[str, list[onnx.NodeProto]] = defaultdict(list)
    for node in model.graph.node:
        for name in node.input:
    ...
    if fused == 0:
        raise ValueError(
            f"No generator Conv+AdaIN patterns found in ONNX model: {input_path}"
        )
...
def _find_adain_after_conv(
    ...
    if {node.op_type for node in first} != {"ReduceMean", "Sub"} or len(first) != 2:
        return None
    reduce_mean = next((node for node in first if node.op_type == "ReduceMean"), None)
    sub = next((node for node in first if node.op_type == "Sub"), None)
