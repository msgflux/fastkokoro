...
def test_cli_build_conv_custom_op_sets_conv_adain_environment(monkeypatch, tmp_path):
    output = tmp_path / "libfastkokoro_conv_adain.so"
    ...
    with (
        ...
        patch(
            "fastkokoro.cli.build_conv_adain_custom_op",
            return_value=output,
        ) as build,
        patch("fastkokoro.cli.uvicorn.run") as run,
    ):
        cli.main()
        assert cli.os.environ["FASTKOKORO_ONNX_CONV_ADAIN_FUSION"] == "true"
        assert cli.os.environ["FASTKOKORO_ONNX_CONV_ADAIN_CUSTOM_OP_LIBRARY"] == str(
            output
        )
        assert cli.os.environ["FASTKOKORO_ONNX_PROVIDERS"] == "CPUExecutionProvider"
